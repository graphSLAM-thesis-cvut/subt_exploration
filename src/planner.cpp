...
public:
    ValidityChecker(ob::SpaceInformationPtr &si, Eigen::MatrixXf &traversability, Eigen::MatrixXi &explored, float slope_th, pairs start_coord, Eigen::MatrixXf& clarity) : ob::StateValidityChecker(si)
    {
    ...
    double clearance(const ob::State *state) const
    {
        ...
        // radius
        return clearity_->coeff(x, y);
    }

    bool get_obstacles()
    {
        // detect all obstacles
        std::vector<pairs> obsts;
        ...
        while (!q_m.empty())
        {
            ...
            for (int t = 0; t < N_S4; t++)
            {
                ...
                if (cell_states[ij] == MAP_OPEN_LIST || cell_states[ij] == MAP_CLOSE_LIST) // ignore used cells
                    continue;
                if (explored_->coeff(i, j) && traversability_->coeff(i, j) >= 0 && traversability_->coeff(i, j) <= slope_th_)
                { // propagate through clear cells
                    // (*clearity_)(i, j) = 0;
                    q_m.push(ij);
                    cell_states[ij] = MAP_OPEN_LIST;
                }
                if (explored_->coeff(i, j) && traversability_->coeff(i, j) >= slope_th_)
                { // add obstacles
                    obsts.push_back(ij);
    ...
    bool get_clarities()
    { // call only after you call get_obstacles
        // detect all obstacles
        //  std::vector<pairs> obsts_;
        //  auto check_cell = start_coords_;
        //  clearity_(check_cell.first, check_cell.second) = 0;
        ...
        while (!q_m.empty())
        {
            ...
            for (int t = 0; t < N_S4; t++)
            {
                ...
                if (cell_states[ij] == MAP_OPEN_LIST || cell_states[ij] == MAP_CLOSE_LIST) // ignore used cells
                    continue;
                if (explored_->coeff(i, j) && traversability_->coeff(i, j) >= 0 && traversability_->coeff(i, j) <= slope_th_)
                { // propagate through clear cells, and add clarity there
                    // (*clearity_)(current_i, current_j) = 0;
                    q_m.push(ij);
                    cell_states[ij] = MAP_OPEN_LIST;
...
std::vector<std::pair<int, int>> plan(std::pair<int, int> startCoord, std::pair<int, int> goalCoord,
                                      ...
                                      Eigen::MatrixXf &traversability, Eigen::MatrixXi &explored, float slope_th, Eigen::MatrixXf &clarity, int upsampling_distance)
{
    ...
    si->setStateValidityChecker(std::make_shared<ValidityChecker>(si, traversability, explored, slope_th, startCoord, clarity));
