				...
				if (is_frontier_point(h_diffs_expanded, explored, n_cell, travTh))
				{
					...
					for (int t = 0; t < N_S; t++)
					{
						...
						if (is_index_valid(i, j, h_diffs_expanded))
						{
							...
							{
								// ROS_INFO("wfd 4");
								if (h_diffs_expanded(i, j) < travTh)
								{ // if(h_diffs_expanded(i, j) != 100) {
									// ROS_INFO("wfd 5");
									q_f.push(ij);
				...
				if (cell_states[ij1] != MAP_OPEN_LIST && cell_states[ij1] != MAP_CLOSE_LIST && h_diffs_expanded(i1, j1) < travTh && h_diffs_expanded(i1, j1) >= 0)
				{
					...
					for (int t2 = 0; t2 < N_S4; t2++)
					{
						...
						if (is_index_valid(i2, j2, h_diffs_expanded))
						{
							...
							// ROS_INFO("wfd 6.3");
							if (h_diffs_expanded(i2, j2) < travTh && h_diffs_expanded(i2, j2) >= 0 && explored(i2, j2))
							{ //>= 0 AANPASSING
								// ROS_INFO("wfd 6.4");
...
void expand(const Eigen::MatrixXf &h_diffs, Eigen::MatrixXf &h_diffs_expanded, const Eigen::MatrixXi &explored, int posex, int posey, int robot_size_cells, float travTh, bool unknownIsObstacle = false)
{
	...
	int v_neighbours[N_S4];
	//
	ROS_INFO("expanding the map's obstacles");
	while (!q_m.empty())
	{
		// ROS_INFO("wfd 2");
		pairs cur_pos = q_m.front();
		...
		for (int t1 = 0; t1 < N_S4; ++t1)
		{
			...
			pairs ij1(i1, j1);
			// ROS_INFO("wfd 6");
			if (is_index_valid(i1, j1, h_diffs) && cell_states[ij1] != MAP_CLOSE_LIST && cell_states[ij1] != MAP_OPEN_LIST) // any valid unvisited cell
			{
				h_diffs_expanded(i1, j1) = h_diffs(i1, j1); // filling the values on the way
				if (explored(i1, j1) == 1 && h_diffs(i1, j1) < travTh && h_diffs(i1, j1) >= 0)
				{ // propagate through the traversable cells
					q_m.push(ij1);
					cell_states[ij1] = MAP_OPEN_LIST;
				}
				else if (explored(i1, j1) == 1 && h_diffs(i1, j1) > travTh) // increase boundaries if obstacle
					increase_boudary(h_diffs, h_diffs_expanded, explored, i1, j1, robot_size_cells, cell_states);
				else if ((explored(i1, j1) == 0 || h_diffs(i1, j1) == -1.0) && unknownIsObstacle) // increase boundaries if unknown and planning
					increase_boudary(h_diffs, h_diffs_expanded, explored, i1, j1, robot_size_cells, cell_states);
			}
		}
		// ROS_INFO("wfd 7");
...
{
	float value = 100;
	if (explored(row, col) && h_diffs(row, col) > 0)
	...
	for (int di = -robot_size_cells; di <= robot_size_cells; di++)
	{
		for (int dj = -robot_size_cells; dj <= robot_size_cells; dj++)
		{
			...
			if (!is_index_valid(cellX, cellY, h_diffs))
				continue;
			if (di * di + dj * dj > robot_size_cells * robot_size_cells - 1)
